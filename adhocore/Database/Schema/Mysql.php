...
class Mysql extends \Adhocore\Database\Schema {
	...
	public function update_table($table)
	{
		if ($this->fields) {
			...
			$fields = array_map(array($this->driver(), 'wrap'), $this->driver()->list_fields($table));	
	...
	public function drop_field($table, $field)
	{
		$drop = "ALTER TABLE {$this->driver()->wrap_table($table)} DROP COLUMN "
				.implode(', DROP COLUMN ', array_map(array($this->driver(), 'wrap'), (array) $field));
	...
	protected function attrib_sql($asString = FALSE)
	{
		...
		if ($this->attribs) {
			foreach ($this->attribs as $type => $attrib) {
				if ($type == 'primary') {
					$attrs[$type] = 'PRIMARY KEY ('.implode(', ', array_map(array($this->driver(), 'wrap'), array_unique($attrib))).')'; 

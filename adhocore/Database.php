...
class Database {
	...
	public function queries($query = array())
	{
		if (isset($query['sql']))
			$this->queries[] = $query;
	...
	public function insert($table, $data)
	{
		...
		$sql = "INSERT INTO {$this->wrap($table)} (" . implode(array_map(array($this, 'wrap'), $fields), ", ") . ") "
	...
	public function insert_batch($table, $datas)
	{
		...
		$sql = "INSERT INTO {$this->wrap_table($table)} (" . implode(array_map(array($this, 'wrap'), $fields), ", ") . ") VALUES "
	...
	public function update($table, $data, $where, $bind = "") 
	{
		ksort($data);
		$fields = $this->filter_fields($table, $data);
		$bind   = (is_array($bind)) ? array_flatten($bind) : (empty($bind) ? array() : array($bind));
		
		$sql = "UPDATE {$this->wrap_table($table)} SET "
			 . implode('= ? , ', array_map(array($this, 'wrap'), $fields)).' = ? '
